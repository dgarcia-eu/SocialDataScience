...
nodesdf$name[nodesdf$id==topedge$from]
nodesdf$name[nodesdf$id==topedge$to]

inner_join(topedge, nodesdf, by=c("from" = "id")) %>%
  inner_join(nodesdf, by=c("to"="id")) -> edf
...
```{r, message=F}
library(ggraph)
ggraph(starwars) + geom_node_point() + geom_edge_link()  +  theme_graph()
```
...
```{r}
starwars %>% ggraph(layout = "circle")  + geom_edge_link(color="gray") + geom_node_point(colour="lightblue", size=3) + geom_node_text(aes(label=name), repel=T) +  theme_graph()
```
