...
**Calculate the reminder of 10 divided by 7 (i.e. 10 modulo 7). (Hint: search in Google or Stackoverflow)**
```{r}
# Your code here
```
...
**Your turn: **
**Iterate over the numbers from 1 to 50 and print the ones divisible by 7.**
```{r}
#Your code here
```
...
Another example:
Open the file "TutorialHeightsSurvey.dat" in a plain text reader to see its content. Then load it into R and name it SurveyDF. Print its first 7 lines.
...
Another example: Print the heights of the rows in SurveyDF of gender "female".
```{r}
