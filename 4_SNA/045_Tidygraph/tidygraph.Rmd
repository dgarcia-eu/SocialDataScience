...
You can generate other kinds of graphs, some of them with a random component. You can learn more about these models in the [network models]() tutorial.
...
You can do similar operations with other dplyr verbs. For example, here we first operate on the edges to remove the ones with weight lower than or equal to 10, then caculate node degree in the resulting network, and plot with the nodes that had degree larger than zero:
...
You can learn more about how to customize ggraph visualization in [this tutorial](http://mr.schochastics.net/netVizR.html). For a more interactive way of visualizing networks, especially large ones, check our [tutorial on visualization with Gephi]().