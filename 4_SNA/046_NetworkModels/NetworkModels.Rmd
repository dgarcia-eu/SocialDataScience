...
Just with a probability of 0.1, the result is pretty different. This model can generate networks with a high clustering coefficient and a low average path length, similar as some social networks you can learn about in the [structural holes topic]().
...
Sometimes we want to compare a social network to a random one that has the same sequence of degrees. This allows us to see if some other property can be explained by the degree sequence alone, as we do in the [permutation tests topic](). Tidygraph helps for this with the play_degree() function, which generates networks of fixed degree sequences but with edges distributed at random. For example:
